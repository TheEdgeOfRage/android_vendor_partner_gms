...
def update_if_needed(module: str, release: ApkRelease):
    ...
    with open(path.join(module_dir, '.version_code'), 'r+') as version_code_file:
        ...
        if version_code < release.version_code:
            ...
            if old_sig != new_sig:
                raise Exception('Signature mismatch for {} old sig: {} new sig: {}'.format(
                    module, old_sig, new_sig))
            ...
            print('{} ahead of suggested version ({} > {})'.format(
                module, version_code, release.version_code
            ))
        elif version_code == release.version_code:
...
# update_if_needed('FDroidPrivilegedExtension', fdroid_recommended_release(
#     fdroid_main_repo, 'org.fdroid.fdroid.privileged'))
# update_if_needed('GmsCore', fdroid_recommended_release(
#     fdroid_microg_repo, 'com.google.android.gms'))
# update_if_needed('GsfProxy', fdroid_recommended_release(
#     fdroid_microg_repo, 'com.google.android.gsf'))
# update_if_needed('IchnaeaNlpBackend', fdroid_recommended_release(
#     fdroid_main_repo, 'org.microg.nlp.backend.ichnaea'))
# update_if_needed('NominatimGeocoderBackend', fdroid_recommended_release(
#     fdroid_main_repo, 'org.microg.nlp.backend.nominatim'))
